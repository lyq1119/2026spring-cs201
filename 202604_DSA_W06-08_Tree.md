...
*Updated 2026-04-19 22:52 GMT+8*  
...
接下来实现 insert 方法。将元素加入列表的最简单、最高效的方法就是将元素追加到列表的末尾。追加操作的优点在于，它能保证完全树的性质，但缺点是很可能会破坏堆的结构性质。不过可以<mark>写一个方法，通过比较新元素与其父元素来重新获得堆的有序性</mark>。如果新元素小于其父元素，就将二者交换。图3展示了将新元素放到正确位置上所需的一系列交换操作。
...
1. **并查集 (Disjoint Set)**：在并查集中，每个元素都属于一个集合，集合之间互不相交。为了高效实现，我们通常使用**树形结构**来表示集合。**每个集合由一棵树表示，树的根节点是该集合的代表元素 (Representative)**。在实现上，通常用一个数组记录每个节点的父节点（这种方式可以看作是简化版的邻接表），从而直观地维护元素间的隶属关系。

2. **前缀树 (Trie Tree)**：用于存储字符串集合。每个节点代表一个字符，从根到叶子的路径表示一个完整字符串。为了表示这种多叉树结构，通常使用邻接表（或数组/字典）存储子节点指针。

**总结**：并查集侧重于“动态合并与连通性判断”，前缀树侧重于“前缀匹配与检索”。

---

### 9.2 并查集 (Union-Find Algorithm) 详解

#### 9.2.1 什么是并查集？

如果两个集合没有共同元素（交集为空），则称它们为**不相交集合 (Disjoint Sets)**。
**不相交集合数据结构**支持以下核心操作：

- **添加 (Add/Init)**：向集合中添加新元素。
- **合并 (Union)**：将两个集合合并为一个。
- **查找 (Find)**：找到一个元素所在集合的代表元素。
- **检查 (Check)**：判断两个元素是否属于同一集合（即是否连通）。

#### 9.2.2 应用场景：朋友圈问题

考虑 10 个人 (a, b, c, d, e, f, g, h, i, j)，已知以下好友关系：

- `a <-> b`, `b <-> d`, `c <-> f`, `c <-> i`, `j <-> e`, `g <-> j`

**目标：** 判断 `a` 和 `d` 是否是朋友（直接或间接）。
通过并查集，我们可以构建出 4 个分组：
...
**判定逻辑：** 如果两个人的**代表元素**相同，则他们在同一组中。

#### 9.2.3 核心数据结构

- **数组 `Parent[]`**：`Parent[i]` 存储第 `i` 个项的父节点。这些关系构成了森林（多棵虚拟树）。
- **代表元素 (Root)**：树的根节点。识别规则：若 `Parent[i] == i`，则 `i` 是该集合的代表。若不是根，则递归向上寻找。
...
- **时间复杂度**：结合路径压缩与按秩合并后，均摊复杂度接近常数，即 $O(\alpha(n))$，其中 $\alpha$ 是反阿克曼函数（对于地球上观测到的所有 $n$，该值通常不大于 4）。
...
并查集不只是一个模板，它是**动态维护等价关系**的强力工具。

- **核心操作**：`find`（找Boss）与 `union`（帮派合并）。
- **关键优化**：路径压缩（最重要）与按秩合并。
- **本质**：管理不相交集合，高效处理连通性问题。





## A 非递归线段树（Iterative Segment Tree）
...
- **与其他算法对比**：在力扣上，很多线段树能做的题，**树状数组（Binary Indexed Tree / Fenwick Tree）** 也能做。树状数组代码更短、常数更小，但线段树的优势在于它的**通用性**（不仅能做求和，还能做区间最大公约数、区间最值、甚至是复杂的动态规划优化）。
...
这是一份经过润色和优化的 **前缀树（Trie Tree）** 教程。我优化了语言表达，使其更符合中文技术文档的习惯，并对代码和逻辑进行了梳理。
