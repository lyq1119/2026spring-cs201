...
*Updated 2026-04-14 14:54 GMT+8*  
...
虽然此处不给出严谨的正确性证明，但其**核心直觉**源于一个简单的推论：在任何最优编码方案中，频率最低的两个字符（设为 $a$ 和 $b$）的码字应当仅在最后一位比特有所不同。通过将 $a, b$ 合并为一个新字符 $c$，并在规模缩减后的字符集上重复这一逻辑，即可推导出最优性：

**命题（Proposition）**：对于长度为 $n$、包含 $d$ 个不同字符的字符串，哈夫曼算法可以在 $O(n + d \log d)$ 时间内构造出一棵**最优前缀码（Optimal Prefix Code）**树。



#### 4.3.2 贪心法 (The Greedy Method)

哈夫曼算法是<mark>**贪心法（Greedy Method）**这一算法设计模式的典型应用。贪心法常用于解决优化问题，即通过在构建结构的过程中不断最小化或最大化某个属性，从而寻求全局最优解。</mark>

贪心法的核心思路（或“设计范式”）与暴力法（Brute-force）同样直观。为了解决某个优化问题，贪心法通常通过一系列**局部决策**来逐步构建解：

1.  **初始状态**：从一个定义明确的初始状态出发，并计算其初始代价（Cost）。
2.  **局部最优选择**：在每一步决策中，从当前所有可行的选项中，挑选出那个**能带来最大即时收益**（或最小代价改进）的选项。
3.  **迭代执行**：持续上述选择，直至无法继续改进或达到终止条件。

**值得注意的是，贪心法并不总能得到全局最优解。** 但对于特定类别的问题，这种“局部最优”的累积确实能导向“全局最优”。这类问题被认为具有**贪心选择性质（Greedy-choice Property）**：即通过一系列局部最优决策，最终能够构建出全局最优解。<mark>求解最优可变长度前缀编码的问题，正是具备贪心选择性质的经典案例。</mark>
